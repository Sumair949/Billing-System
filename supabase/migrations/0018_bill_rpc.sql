-- create_bill_with_items: inserts a bill + its items atomically.
-- security definer so workers (p_user_id != auth.uid()) can create
-- bills on behalf of their owner after the relationship is verified.
...
declare
    ...
    v_sr_no   int := 1;
begin
    -- Workers must be active and linked to the target owner
    ...
    insert into public.bills (
        ...
        discount,        prepared_by,      approved_by
    ) values (
        p_user_id,
...
grant execute on function public.create_bill_with_items(uuid, jsonb, jsonb) to authenticated;


-- upsert_bill_with_items: updates a bill and atomically replaces its items.
-- security invoker so RLS ensures the caller can only touch their own bills.
...
grant execute on function public.upsert_bill_with_items(uuid, jsonb, jsonb) to authenticated;
