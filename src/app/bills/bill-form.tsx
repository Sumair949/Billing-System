...
function zeroIfBlank(v: string | undefined): string {
    const s = (v ?? "").trim();
    return s === "" ? "0" : s;
}
...
export function BillForm({ action, submitLabel, defaultValues }: Props) {
    ...
    const autoTotal = useMemo(() => {
        const itemsSum = items.reduce((sum, it) => {
            const w = safeNumber(it.weight);
            const q = safeNumber(it.quantity);
            const r = safeNumber(it.rate);
    ...
    const [totalOverride, setTotalOverride] = useState<string | null>(() => {
        ...
        const initialAuto =
            (defaultValues.items ?? []).reduce((sum, it) => {
                const w = safeNumber(it.weight ?? "");
                const q = safeNumber(it.quantity ?? "");
                const r = safeNumber(it.rate);
                return sum + (w > 0 ? w : q) * r;
    ...
    return (
        <form action={formAction} className="space-y-6" noValidate>
            ...
            <SectionCard
                ...
                title="Line items"
                description="Enter either Qty or Weight per row — not both. Amount is calculated automatically."
                action={
                    <button
                        type="button"
                        onClick={addItem}
            ...
            >
                <div className="grid gap-4 md:grid-cols-3">
                    <div className="rounded-lg bg-header px-4 py-3 text-header-foreground shadow-sm">
                        ...
                        <div className="mt-1">
                            <FieldError message={state.fieldErrors?.total_amount} />
                        </div>
                    </div>
