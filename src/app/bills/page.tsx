...
async function BillStats() {
    const supabase = await createSupabaseServerClient();
    const { data } = await supabase.rpc("bill_stats");
    const stats = (data as StatsRow | null) ?? {
        total_count: 0,
        total_revenue: "0",
        outstanding: "0",
        this_month_count: 0,
    ...
    return (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <StatCard label="Total bills" value={stats.total_count.toLocaleString()} />
            <StatCard label="Total revenue" value={formatAmount(stats.total_revenue)} />
            <StatCard
                label="Outstanding"
                value={formatAmount(stats.outstanding)}
                emphasis={Number(stats.outstanding) > 0 ? "warning" : undefined}
            />
            <StatCard label="This month" value={stats.this_month_count.toLocaleString()} />
        </div>
    );
}
...
async function BillsTable({
    ...
    if (q) {
        ...
        listQuery = listQuery.or(`bill_no.ilike.${pattern},customer_name.ilike.${pattern}`);
    }

    const bounds = rangeBounds(range);
    if (bounds) {
        listQuery = listQuery.gte("created_at", bounds.from.toISOString());
        if (bounds.to) listQuery = listQuery.lt("created_at", bounds.to.toISOString());
    }
        ...
        <div className="overflow-hidden rounded-xl bg-surface shadow-sm ring-1 ring-border">
            <div className="flex flex-col gap-3 border-b border-border p-4 sm:p-5">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <h2 className="text-base font-semibold text-foreground">All bills</h2>
                        ...
                        <table className="w-full min-w-[720px] text-sm">
                            ...
                            <tbody className="divide-y divide-border">
                                {bills.map((bill) => {
                                    ...
                                    return (
                                        <tr key={bill.id} className="transition hover:bg-muted/40">
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center justify-end gap-4">
                                                    ...
                                                    <Link
                                                        href={`/print/ledger/${encodeURIComponent(bill.customer_name)}`}
                                                        target="_blank"
                                                    ...
                                                    <DeleteBillButton id={bill.id} billNo={bill.bill_no} />
                                                </div>
                                            </td>
                                        </tr>
                                    );
        ...
        <>
            ...
            <section className="space-y-8">
                ...
                <Suspense fallback={<TableSkeleton />}>
                    <BillsTable q={q} page={page} range={range} status={status} />
                </Suspense>
            </section>
        </>
    );
...
function EmptyState({
    ...
    return (
        <div className="p-16 text-center">
            {hasFilters ? (
                <p className="text-sm text-muted-foreground">No bills match your current filters.</p>
            ) : (
                <>
