...
import { deriveStatus, type BillStatus, type Purchase } from "@/lib/supabase/types";
...
async function PurchaseStats() {
    ...
    const { data } = await supabase.rpc("purchase_stats");
    const raw = Array.isArray(data) ? (data as unknown[])[0] : data;
    const stats = (raw as StatsRow | null) ?? {
        total_count: 0,
        total_spend: "0",
        outstanding: "0",
        this_month_count: 0,
    ...
    return (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <StatCard label="Total purchases" value={stats.total_count.toLocaleString()} />
            <StatCard label="Total spend" value={formatAmount(stats.total_spend)} />
            <StatCard
                label="Outstanding"
...
async function PurchasesTable({
    ...
    if (q) {
        ...
        listQuery = listQuery.or(`purchase_no.ilike.${pattern},supplier_name.ilike.${pattern}`);
    }
        ...
        <div className="overflow-hidden rounded-xl bg-surface shadow-sm ring-1 ring-border">
            <div className="flex flex-col gap-3 border-b border-border p-4 sm:p-5">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <h2 className="text-base font-semibold text-foreground">All purchases</h2>
            ...
            {purchases.length === 0 ? (
                ...
                <>
                                        ...
                                        <tr key={p.id} className="transition hover:bg-muted/40">
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center justify-end gap-4">
                                                    ...
                                                    <DeletePurchaseButton id={p.id} purchaseNo={p.purchase_no} />
                                                </div>
                                            </td>
                                        </tr>
                                    );
                    ...
                    {totalPages > 1 ? (
                        <Pagination page={page} totalPages={totalPages} q={q} status={status} total={total} />
                    ) : (
        ...
        <>
            ...
            <section className="space-y-8">
                <header className="flex flex-wrap items-end justify-between gap-4">
                    <div>
                        <h1 className="text-3xl font-bold tracking-tight sm:text-4xl">Purchases</h1>
                ...
                <Suspense fallback={<TableSkeleton />}>
                    <PurchasesTable q={q} page={page} status={status} />
                </Suspense>
            </section>
        </>
    );
